...
func TestStatefulSet_TemplateSpec(t *testing.T) {
	...
	assert.Equalf(t, instance.NeedsStatsD(), volumeIsDefined(templateSpec.Volumes, "auth-tokens"),
...
func TestStatefulSet_Container(t *testing.T) {
	...
	assert.Equalf(t, instance.NeedsStatsD(), len(container.VolumeMounts) > 0,
		"Expected that there are no volume mounts iff StatsD is disabled",
	)
	assert.Equalf(t, instance.NeedsStatsD(), mountPathIsIn(container.VolumeMounts, "/var/lib/dynatrace/gateway/config"),
